...
class HarFileIO(object):
    ...
    def readHarFileInfo(filename: str) -> HarFileInfoObj:
        ...
        with open(filename, "rb") as f:
            ...
            while True:
                ...
                hfi=hfiObj.ha_infos[name.upper()]
    ...
    def _unpack_data(fp, form, data=''):
        ...
        # This fixed what I thought was a bug where the data object was not loaded and thus I manually reconstructed it. Bit by bit. Lol!
        if not data and form[0:2] == '=4':
            data = fp.read(struct.calcsize(form))
            
            # Wow.... manual reconstruction of har file from struct type.
            data = ('    1CFULL' + '5' * 70).encode() + b'\x02\x00\x00\x00'
            raise NameError('If you are here, you did something very wrong.')
    ...
    def writeHeaders(filename: 'Union[str, io.BufferedWriter]', hnames : 'List[str]',
        ...
        with fp:
            for hname, head_arr_obj in zip(hnames, head_arr_objs):
                ...
                if header_type_str == 'int64':
                    head_arr_obj.array = head_arr_obj.array.astype(np.int32)
                
                # if header_type_str == 'float64':
                    # head_arr_obj.array = head_arr_obj.array.astype(np.int32)
                
                if head_arr_obj.dtype == '1C':
                    5
                # GTAP-InVEST addition: if given a direct numpy type, assume it is a string (and further that it is length 12).
                ...
                if header_type_str in ['float32','float64'] and (head_arr_obj.array.ndim != 2 or has_sets):
                    ...
                    if head_arr_obj.array.ndim > 1:
                        raise NameError('"' + hname + '" can not be written as character arrays ndim>1 are not yet supported')                        
    ...
    def _writeHeader2D(fp: BinaryIO, hname : str, head_arr_obj: HeaderArrayObj):
        HarFileIO._writeHeaderName(fp, hname)
        array_type_string = str(head_arr_obj.array.dtype)
        shape2D = [head_arr_obj.array.shape[i] if i < head_arr_obj.array.ndim else 1 for i in range(0, 2)]
        if array_type_string == 'int32':
            dtype = 'i'
            secRecList = ['    ', '2I', 'FULL', head_arr_obj.long_name, 2]
        elif array_type_string == 'float32':
            secRecList = ['    ', '2R', 'FULL', head_arr_obj.long_name, 2]
            dtype = 'f'
        else:
            raise TypeError("Can only write 32bit float or int to 2D arrays. Found this for " + str(fp) + ', ' + str(hname) + ', ' + str(head_arr_obj))
    ...
    def _writeHeader1C(fp: BinaryIO, hname : str, head_arr_obj: HeaderArrayObj):
        ...
        HarFileIO._writeHeaderName(fp, hname)
        array_type_string = str(head_arr_obj.array.dtype)
        hao_type_string = str(head_arr_obj.dtype)

        # GTAP-InVEST addition: if given a direct numpy type, assume it is a string (and further that it is length 12).
        # if 'object' in array_type_string:
        #     array_type_string = '<U12'
                    
        no_chars = int(hao_type_string[2:])
