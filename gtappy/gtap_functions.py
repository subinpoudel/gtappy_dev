            ...
            for har_filename in hars_to_look_for:
                ...
                if hb.path_exists(input_har_path):
                    ...
                    if not hb.path_exists(har_index_path, verbose=True): # Minor note, could add more robust file validation to check for ALL the implied files to exist.
                        ...
                        gtap_invest_integration_functions.har_to_indexed_dfs(input_har_path, har_index_path)                    
                        ...
                        gtap_invest_integration_functions.indexed_dfs_to_har(har_index_path, local_har_path)
        ...
        for aggregation_label in p.aggregation_labels:
            ...
            for experiment_label in p.experiment_labels:
                ...
                if not hb.path_exists(expected_sl4_path):
                    ...
                    inputs_dict = gtap_invest_integration_functions.default_erwin_style_cmf_dict.copy()
                    
                    # UPDATE THE CMF generation dict if 10x10 aggregation (141x65 was the default)
                    if aggregation_label == '10x10':
                        inputs_dict['xSets'] = {
                            'AGCOM': ['Agri commodities', '(GrainsCrops, MeatLstk, ProcFood)'],
                            'AGCOM_SM': ['smaller agri commodities', '(GrainsCrops, MeatLstk)']
                        }
                        inputs_dict['xSubsets'] = ['AGCOM is subset of COMM', 'AGCOM is subset of ACTS', 'AGCOM_SM is subset of COMM', 'AGCOM_SM is subset of ACTS'] 
                        inputs_dict['Shock'] = {
                    ...
                    gtap_invest_integration_functions.generate_erwin_style_gtap_cmf_file_for_scenario(inputs_dict, 
                                                                experiment_label,                                                    
                                                                data_dir,
                                                                output_dir,     
                                                                generated_cmf_path,     )
                    ...
                    if run_parallel: # When running in paralell, add it to a list for later parallel processing.
                        ...
                        call_list = [cge_executable_path, '-cmf', generated_cmf_path]
                        gtap_invest_integration_functions.run_gtap_cmf(generated_cmf_path, call_list)
        ...
        for aggregation_label in p.aggregation_labels:
            ...
            for experiment_label in p.experiment_labels:
                ...
                for filename in expected_filenames:
                    ...
                    experiment_dir = os.path.join(p.run_gtap_v7_executable_dir, aggregation_label, experiment_label)
                    ...
                    if not hb.path_exists(indexed_df_path):                    
                        gtap_invest_integration_functions.har_to_indexed_dfs(expected_path, indexed_df_path)
                    ...
                    if not hb.path_exists(har_path):
                        gtap_invest_integration_functions.indexed_dfs_to_har(indexed_df_path, har_path) 
                    
                    

def gtap_summary_tables(p):
